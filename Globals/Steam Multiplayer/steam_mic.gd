...
var lobby_voice_gens:Dictionary[int,Array] = {}
...
func check_for_voice() -> void:
	...
	if available_voice['result'] == Steam.VOICE_RESULT_OK and available_voice['buffer'] > 0:
		...
		if voice_data['result'] == Steam.VOICE_RESULT_OK and voice_data['written']:
			...
			Network.p2p_send_voice_data(voice_data)
			#Network.send_p2p_packet(0,{"voice":voice_data},2)
			...
			#else:
				#print("Loopback off")
				#process_voice_data(voice_data, "network")
