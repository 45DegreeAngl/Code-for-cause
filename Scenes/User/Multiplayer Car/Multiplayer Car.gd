...
var prox_player : prox_voice_3d

##multiplayer stuff
var drunkeness:float = 20
signal update_bottles(dict:Dictionary)
var car_contents:Dictionary = {"Beer":1,"Sake":0,"Jaeger":0}:
	set(value):
		car_contents = value
		update_bottles.emit(car_contents)

func is_steam_authority()->bool:
	return GlobalSteam.is_steam_authority(steam_id)

@export var radio_node:RadioPlayer
@export var cop_detector:DistanceGadget

# Called when the node enters the scene tree for the first time.
func _ready() -> void:
	original_engine_power = ENGINE_POWER
	car_contents = {"Beer":1,"Sake":0,"Jaeger":0}
	#Globals.player_vehicle = self
	if !is_steam_authority():
		$Cameras/Windshield.current = false
		return

	$Cameras/Windshield.make_current()

	Input.mouse_mode = Input.MOUSE_MODE_CAPTURED
	radio_node._on_radio_finished()
...
func _process(delta: float) -> void:
	...
	Network.p2p_call_func(get_path(),"update_car_data",[steering,engine_force],0)

func update_car_data(steer,force):
	steering = steer
	engine_force = force
...
func _input(event: InputEvent) -> void:
	if !is_steam_authority() or Globals.game_paused:
		return
